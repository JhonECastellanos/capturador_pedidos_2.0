...
export default function PedidoCompletado() {
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div className="flex-shrink-0 space-y-2.5 border-t border-line bg-paper-raised px-5 pb-[max(1.25rem,env(safe-area-inset-bottom))] pt-4 md:px-6"><Boton onClick={() => navegar("/modulos/pedido-rapido", { state: { clienteId: cliente.id } })}>Capturar otro pedido</Boton><Boton variante="fantasma" onClick={() => navegar("/modulos/vendedor")}>Volver al inicio</Boton></div>
    </div>
  );
}
